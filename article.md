...
Finally you fire up a webkit browser such as chrome or safari. and go to [http://127.0.0.1:8080/debug?port=5858](http://127.0.0.1:8080/debug?port=5858). Note, if the debugger is listening on a port other than `5858`, you will need to change it. Also, if you passed a custom `wep-port` argument to node-inspector, then you will have to modify the `8080`.
